...
with tf.device('/gpu:0'):
    ...
    sen2vec = Bidirectional(GRU(300, activation='tanh', return_sequences=False))(wordembed)    
    conv_out = Dropout(0.5)(sen2vec)
    model_output = Dense(8, activation='softmax')(conv_out)
    model = Model(inputs=model_input, outputs=model_output)
    model.summary()
    checkpoint = ModelCheckpoint('../../PCmodel/Word2VecBiGRU.h5', monitor='val_categorical_accuracy', verbose=1, save_best_only=True, mode='max')
    ...
    model.save('../../PCmodel/Word2VecBiGRU.h5')
